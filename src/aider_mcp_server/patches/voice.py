...
# Only import audio libraries if audioop is available
...
class Voice:
    ...
    def __init__(self, audio_format="wav", device_name=None):
        # Check if required audio modules are available
        if not PYDUB_AVAILABLE:
            raise SoundDeviceError("PyDub unavailable - audio module 'audioop' not found")
        
        if not SF_AVAILABLE:
            raise SoundDeviceError("SoundFile module not available")
        ...
        try:
            ...
            raise SoundDeviceError("SoundDevice module not available")
...
def audio_available():
    """Return whether voice recording functionality is available."""
    return AUDIOOP_AVAILABLE and PYDUB_AVAILABLE and SF_AVAILABLE


if __name__ == "__main__":
    if not audio_available():
        print("Audio functionality not available. Missing required modules.")
        exit(1)
